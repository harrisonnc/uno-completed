...
public class UNOPanel extends javax.swing.JPanel {
	...
	public void solveScore()
        {
            ...
            {  
                    test[0]=("Player"+Integer.toString(lcv+1)); 
                    test[1]=Integer.toString(scores[lcv]);
                    model.addRow(test);
            }
            scoreTable=new JTable(model);
            JScrollPane pane = new JScrollPane(scoreTable);    
            pane.setBounds(parentFrame.getWidth()-180,(parentFrame.getHeight()/3)-30,150,120);
            add(pane);
            pane.repaint();
        }
        private void setScores(int winner)
        {
            int score=scores[winner];
            for(int lcv=0;lcv<game.getNumberPlayers();lcv++)
            {
                UNODeck hand = (UNODeck)game.players.get(lcv);
                if(lcv!=winner)
                {
                    for(int lcv1=0;lcv1<hand.getSize();lcv1++)
                    {
                        if(hand.showCard(lcv1).isNumber())
                            score+=hand.showCard(lcv1).getValue();
                        else if(hand.showCard(lcv1).isSkip()||hand.showCard(lcv1).isReverse()||hand.showCard(lcv1).isDrawTwo())
                            score+=20;
                        else
                            score+=50;
                    }
            ...
            scores[winner]=score;
            model.setValueAt(score, winner-1, 1);
            if(scores[winner]>=500)
            {
                int reply = JOptionPane.showConfirmDialog(null, "Player "+winner+" is the has gotten 500 points and is Overall Series Winner!\nDo you want to continue playing with a new series?", "Series Winner!!!",  JOptionPane.YES_NO_OPTION);
                if (reply == JOptionPane.YES_OPTION)
                {
                    
                    for(int lcv=0;lcv<6;lcv++)
                    {
                        model.setValueAt(0, lcv, 1);
                        scores[lcv]=0;
                    }    
                    played.clear();
	...
	private void unoButtonActionPerformed(ActionEvent evt) {
		...
		new JOptionPane("Player 1 called Uno!", JOptionPane.PLAIN_MESSAGE, JOptionPane.DEFAULT_OPTION).createDialog(this, "Someone has called Uno!").setVisible(true);
		unoTimer.stop();
		repaint();
	...
	private Rectangle[] getCardPositions(int cardsWide, int cardLevel, int cardMaxLevels) {
		...
		int y = (parentFrame.getHeight() - cardMaxLevels*ySpace - cardMaxLevels*cardHeight)/2
		        + cardLevel*ySpace + cardLevel*cardHeight - 10; 
	...
	private class UNOAIListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
			...
			if (result > 28) {
				result -= 35;
                                setScores(game.getLastPlayer() + 1);
				announceWinner(game.getLastPlayer() + 1);
				return;
			} 
                        else if (result > 13) {	
