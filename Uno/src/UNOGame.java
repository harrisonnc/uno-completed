...
/**
  "get" - Is the pile where you get the cards
  "discard" - Is the playing pile
  "players" - Are the hands of the players, considered also as CardPiles
 */
...
public class UNOGame implements GameMessages {
	...
	private Turn turn;         //Tells the player in turn
	...
	*/
	...
	/**
	  Plays the card with that number in the current player's deck, you
	  introduce the player that is requesting the action, and the number
	  of the card in his deck (there is one special number that is not a
	  card number: The size of the hand, that indicates that the player
	  is requesting to draw a card).   This method will do the requested
	  action, if valid, and will return a number according to the result
	  of the action:
	  
	  GAME_FINISHED - Game's finished, you can't do anything else
	  NOT_YOUR_TURN - It's not players turn yet!!
	  SELECT_COLOR_FIRST - The game need to determine the color of the next card
	  DECK_EMPTY - The player tried to draw a card, but deck's empty (pass turn)
	  DREW_AND_VALID - The player drew a card and has a valid play
	  DREW_AND_NO_VALID - The player drew a card and passed his turn
	  CARD_NOT_VALID - The card selected was not valid and nothing happened
	  PLAYED_NORMAL - Valid card played: A normal card
	  PLAYED_REVERSE - Valid card played: Reverse card
	  PLAYED_SKIP - Valid card played: Skip card
	  PLAYED_DRAW_2 - Valid card played: Draw Two card
	  PLAYED_WILD - Valid card played: Wild card
	  PLAYED_WILD_4 - Valid card played: Wild Draw Four card
	  PLAYED_ZERO - Played a zero and the zero rule is on
	  PLAYED_DRAW_2_N - Valid card played: Draw Two, and the next player has a D2 or W4
	  PLAYED_WILD_4_N  - Valid card played: Wild four, adn the next player has a W4
	  +EXISTS_UNO - The previous player has an uno
	  +EXISTS_WINNER - The previous player is a winner
	  PLAYER_RETIRED+numPlayer - numPlayer has retired from the game
	  
	  In cases 4 and 5, you must call the setColor() method to set the 
	  new color, otherwise this method will return -5.
	  */
